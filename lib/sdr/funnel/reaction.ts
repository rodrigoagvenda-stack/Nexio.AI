...
import type { Categoria, FunnelAction, FunnelState, Reading } from './types'
...
export function structuralChecks(p: {
  ...
  if (frase.trim().split(/\s+/).length > MAX_WORDS) return 'longa_demais'
...
export async function buildReaction(p: {
  ...
  const frase = await writeReaction({ transcript: p.transcript, leadText: p.leadText, openai: p.openai, onUsage: p.onUsage })
  if (!frase) return { texto: null, frase, motivo: 'sem_frase' }

  const forma = structuralChecks({ frase, leadText: p.leadText, lastQuestion: p.lastQuestion, recentOutbound: p.recentOutbound })
