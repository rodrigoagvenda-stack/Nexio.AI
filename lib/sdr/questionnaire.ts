...
// Geração da base de conhecimento e da base de objeções a partir das respostas do assistente de criação.
// Usada pelo assistente (/api/sdr/flows/[id]/agent/build) e pela rota from-questionnaire.
...
export async function generateConhecimento(
  openai: OpenAI,
...
(Se este bloco contiver 'REGRA DE PREÇO', copie essa regra LITERALMENTE no campo precos : é uma trava, não resuma nem reescreva.)
...
export async function generateObjecoes(
  openai: OpenAI,
  answers: QuestionnaireAnswers,
  variables: SdrVariables
): Promise<string> {
