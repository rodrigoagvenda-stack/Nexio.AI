...
async function metaSendText(phoneNumberId: string, token: string, to: string, text: string): Promise<SendResult> {
...
async function metaSendMedia(phoneNumberId: string, token: string, to: string, type: string, fileUrl: string, caption?: string, filename?: string): Promise<SendResult> {
...
export async function sendText(input: SendTextInput): Promise<SendResult> {
  ...
  if (config?.whatsapp_provider === 'meta' && config.meta_wa_phone_number_id && config.meta_wa_token) {
    return metaSendText(config.meta_wa_phone_number_id, config.meta_wa_token, input.phoneNumber, input.text)
  }
...
export async function sendMedia(input: SendMediaInput): Promise<SendResult> {
  ...
  if (config?.whatsapp_provider === 'meta' && config.meta_wa_phone_number_id && config.meta_wa_token) {
    return metaSendMedia(config.meta_wa_phone_number_id, config.meta_wa_token, input.phoneNumber, input.type, input.fileUrl, input.caption, input.filename)
  }
