...
function TestForm() {
  ...
  const webhookUrl = searchParams.get('wh') ? atob(searchParams.get('wh')!) : ''
  ...
  if (!webhookUrl) {
    return (
      <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center space-y-2">
        <p className="text-sm text-red-400">Webhook não configurado. Abra este formulário pelo Admin → Trial SaaS.</p>
      </div>
    )
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(webhookUrl, {
        ...
        body: JSON.stringify({ name: nome, email, whatsapp: telefone.replace(/\D/g, '') }),
      })
