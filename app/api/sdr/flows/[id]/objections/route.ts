...
export async function GET(
  ...
  try {
    ...
    // Verifica se existe base de objeções para este flow
    const { data: docs, count } = await service
      .from('documents')
      .select('metadata', { count: 'exact' })
      .eq('company_id', userData.company_id)
      .contains('metadata', { flow_id: params.id, doc_type: 'objecoes' })
      .limit(1)

    if (!count) return NextResponse.json({ exists: false })
    const filename = docs?.[0]?.metadata?.filename ?? 'Base de objeções'
