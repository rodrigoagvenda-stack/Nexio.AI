...
export async function POST(req: NextRequest) {
  ...
  if (coex || !phoneNumberId) {
    // CoEx: FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING não traz phone_number_id — busca da WABA
    ...
    console.log(`[meta/connect] CoEx WABA ${wabaId} — números:`, numbers.map((n: any) => n.display_phone_number))
    if (numbers.length > 0) {
  ...
  console.log(`[meta/connect] empresa ${context.companyId} conectada via ${coex ? 'CoEx' : 'Cloud API'} — phone=${phone}`)
  return NextResponse.json({ ok: true, phone, token: longToken, phoneNumberId: finalPhoneNumberId })
}
