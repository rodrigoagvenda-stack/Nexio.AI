...
function WaPreview({ step }: { step: TrialStep }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-[#0b141a] flex flex-col min-h-[200px]">
      <div className="flex items-center gap-2.5 px-3 py-2 bg-[#202c33]">
        <div className="w-6 h-6 rounded-full bg-[#3a4a54] flex items-center justify-center">
      ...
      <div className="flex-1 p-3 flex flex-col justify-end">
        <div className="flex justify-end">
          <div className="bg-[#005c4b] rounded-xl rounded-tr-sm px-3 py-2 max-w-[90%] space-y-1.5 shadow">
            {tipo === 'image' && (
            ...
            {(tipo === 'audio' || tipo === 'ptt') && (
              <div className="flex items-center gap-2 py-1">
                ...
                <div className="flex-1 h-1 rounded-full bg-[#00a884]/40">
            ...
            {tipo === 'document' && (
              <div className="flex items-center gap-2 py-0.5">
                ...
                <p className="text-[11px] text-[#e9edef] truncate">{media.docName || 'arquivo.pdf'}</p>
              </div>
            )}
            {tipo === 'menu' && (
              <div className="space-y-1">
                {text && <p className="text-[12px] text-[#e9edef] leading-relaxed">{text}</p>}
                {(media.choices ?? ['Opção 1']).slice(0, 3).map((c, i) => (
                  <div key={i} className="text-center text-[11px] text-[#53bdeb] border border-[#2a3942] rounded-lg py-1">{c || `Opção ${i + 1}`}</div>
                ))}
              </div>
            )}
            {tipo !== 'menu' && text && (
      ...
      <div className="space-y-3">
        ...
        {tipo === 'document' && (
          <div className="space-y-1.5">
            ...
            <Input value={config.docName ?? ''} onChange={(e) => onChange({ docName: e.target.value })} placeholder="manual.pdf" className="h-9 text-sm" />
          </div>
        )}
      </div>
    )
...
function TrialStepEditor({ step, index, total, onChange, onRemove, isActive }: {
  step: TrialStep; index: number; total: number
  onChange: (p: Partial<TrialStep>) => void; onRemove: () => void; isActive?: boolean
}) {
  const needsText = ['text', 'menu', 'carousel'].includes(step.tipo_mensagem)
  return (
    <div className={cn('rounded-xl border bg-background shadow-sm overflow-hidden transition-all', isActive ? 'border-primary/40' : 'border-border')}>
      <div className="flex items-center justify-between px-4 py-2.5 bg-muted/30 border-b border-border">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold">Passo {index + 1}</span>
          <span className="text-xs text-muted-foreground">·</span>
          <span className="text-xs text-muted-foreground">{step.dia_offset === 0 ? 'Boas-vindas (imediato)' : `Dia ${step.dia_offset}`}</span>
        </div>
        {total > 1 && (
        ...
        {needsText && (
          <div className="space-y-1.5">
            ...
            <Textarea value={step.mensagem} onChange={(e) => onChange({ mensagem: e.target.value })}
              placeholder={`Olá, {nome}! ${step.dia_offset === 0 ? 'Bem-vindo ao trial! 🎉' : 'Como está sendo sua experiência?'}`}
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div key={day} className={cn('bg-background min-h-[72px] p-1.5 space-y-1', isToday && 'bg-primary/5')}>
              ...
              {dayTrials.length > 3 && (
                <p className="text-[10px] text-muted-foreground text-center">+{dayTrials.length - 3}</p>
              )}
            </div>
          )
        })}
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          {(() => {
            ...
            return (
              <div className="space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge className={cn('text-xs border', expired ? 'bg-muted text-muted-foreground border-border' : daysLeft <= 2 ? 'bg-red-500/10 text-red-600 border-red-500/20' : daysLeft <= 5 ? 'bg-amber-500/10 text-amber-600 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20')}>
                    {expired ? 'Expirado' : daysLeft === 0 ? 'Expira hoje' : `${daysLeft} dias restantes`}
                  </Badge>
                  <span className="text-xs text-muted-foreground">Trial de {selected.trial_days ?? 7} dias</span>
                </div>
                <div className="h-2 rounded-full bg-muted overflow-hidden">
                  <div className={cn('h-full rounded-full', expired ? 'bg-muted-foreground' : daysLeft <= 2 ? 'bg-red-500' : daysLeft <= 5 ? 'bg-amber-500' : 'bg-emerald-500')}
                    style={{ width: `${Math.min(100, Math.round((daysGone / (selected.trial_days ?? 7)) * 100))}%` }} />
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                  ...
                  <div><span className="font-medium">Expiração: </span>{exp.toLocaleDateString('pt-BR')}</div>
                  <div><span className="font-medium">Dia: </span>{Math.max(0, daysGone)}/{selected.trial_days ?? 7}</div>
                </div>
              </div>
            )
          })()}
...
export default function TrialPage() {
  ...
  const [form, setForm] = useState<{ nome: string; ativo: boolean; steps: TrialStep[] }>({ nome: '', ativo: true, steps: [EMPTY_STEP()] })
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex gap-5 flex-1 overflow-hidden min-h-0">
            <div className="flex-1 overflow-y-auto space-y-4 pr-1">
              <div className="flex items-end gap-3">
                <div className="flex-1 space-y-1.5">
                  <Label>Nome *</Label>
                  <Input value={form.nome} onChange={(e) => setForm((p) => ({ ...p, nome: e.target.value }))} placeholder="Ex: Trial 7 dias" />
                </div>
              ...
              <div className="flex items-start gap-2.5 p-3 rounded-xl border border-border bg-muted/20 text-xs text-muted-foreground">
                ...
                O passo <strong>Dia 0</strong> é enviado imediatamente após o cadastro. Os demais são processados pelo cron diário.
              </div>
              ...
              <div className="space-y-3">
                ...
                {form.steps.map((step, i) => (
                  <TrialStepEditor key={i} step={step} index={i} total={form.steps.length} isActive={previewIdx === i}
                    onChange={(patch) => updateStep(i, patch)}
                    onRemove={() => { setForm((p) => ({ ...p, steps: p.steps.filter((_, idx) => idx !== i) })); setPreviewIdx(Math.max(0, i - 1)) }} />
                ))}
              </div>
            </div>
            ...
            <div className="w-52 shrink-0 space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">Passo {previewIdx + 1}</p>
              {form.steps[previewIdx] && <WaPreview step={form.steps[previewIdx]} />}
              {form.steps.length > 1 && (
                <div className="flex gap-1 justify-center pt-1">
                  {form.steps.map((_, i) => (
                    <button key={i} onClick={() => setPreviewIdx(i)}
                      className={cn('w-1.5 h-1.5 rounded-full transition-all', previewIdx === i ? 'bg-primary' : 'bg-muted-foreground/30')} />
                  ))}
                </div>
              )}
            </div>
