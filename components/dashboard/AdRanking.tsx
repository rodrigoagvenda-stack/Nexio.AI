...
export function AdRanking({ days }: { days: number }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/reports/cac?days=${Math.max(1, days)}`)
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-4 rounded-[14px] border border-border bg-card px-6 py-6">
      <div className="flex items-center gap-3.5">
        ...
        <div className="flex flex-col gap-0.5"><h3 className="text-xl font-semibold text-foreground">Anúncios que trazem conversa</h3><p className="text-[13px] text-muted-foreground">Conversas iniciadas por anúncio</p></div>
      </div>
      ...
      {!loading && !sold && withConv.length > 0 && (
        <p className="flex items-center gap-2.5 rounded-xl border border-dashed border-border px-4 py-3 text-[13px] text-muted-foreground"><Info className="h-4 w-4 shrink-0" />Vendas por anúncio aparecem quando o lead guardar de qual anúncio veio.</p>
      )}
